...
#     def reset(self):
#         self.U = defaultdict(dict)#defaultdict(lambda: defaultdict(np.complex64))
#         self.U[(0,)*self.num_modes][(0,)*self.num_modes] = 1.0 + 0.0j
#         self.dU = None
#         if self.grad:
#             self.dU = defaultdict(dict)#lambda: defaultdict(lambda: np.zeros_like(self.V, dtype=np.complex64)))
#             self.dU[(0,)*self.num_modes][(0,)*self.num_modes] = np.zeros_like(self.V, dtype=np.complex64)

    
#     def build_tree_single_pair(self, kbuild:Tuple[int], kscan:Tuple[int], building_input:bool):
...
#             self.build_tree_single_pair(kbuild, kscan, self.io.building_input)
